...
**Agent connection — single run:**

1. **Config file** — path to your config file (with `agent_url` set)
2. **Output directory** — where results will be saved (defaults to `./out`)

The provider and model selection steps are skipped — calibrate connects directly to your agent. Results are saved to `external_agent/` inside the output directory.

**Agent connection — benchmark across models:**

To benchmark your agent across multiple models, pass one or more model names with `-m`. Calibrate runs the same test cases once per model, including the model name and provider in each request so your agent can route to the right model.
...
Verify your connection works with a specific model before running:
